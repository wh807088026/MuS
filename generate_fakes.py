...
import os
import torch
from data.dataset import TextDatasetval, TextDataset, FidDataset, TextDatasetText, CollectionTextDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument("--textEncoder", action='store_true', default=True)

    parser.add_argument("--no_writer_loss", action='store_true')
    parser.add_argument("--text_concat", action='store_true', default=True)
    parser.add_argument("--writer_loss_weight", type=float, default=1.0)
    parser.add_argument("--no_ocr_loss", action='store_true')
    parser.add_argument("--no_tid_loss", action='store_true', default=False)
    ...
    # args.output = Path(args.output) / Path(args.dataset) / Path(args.checkpoint).stem
    ...
    model.eval()
    with torch.no_grad():
        model.save_images_for_fid_calculation(args.output, train_loader, 'train', args.allImg, args.realImg)
        model.save_images_for_fid_calculation(args.output, test_loader, 'test', args.allImg, args.realImg)
    print('Done')
