...
from data.dataset import TextDataset, TextDatasetval, TextDatasetText, CollectionTextDataset
from models.model import MuS
...
def main():
    ...
    parser.add_argument("--tid_lr", default=0.00005, type=float)
    ...
    dataset = CollectionTextDataset(
        ...
        collator_resolution=args.resolution, min_virtual_size=339  # IAM 339, CVL 283
    )
    datasetval = CollectionTextDataset(
        args.dataset, 'files', TextDatasetval, num_examples=args.num_examples,
        collator_resolution=args.resolution, min_virtual_size=161  # IAM 161, CVL 27
    )
    ...
    datasettext = CollectionTextDataset(
        args.ttfname, 'files', TextDatasetText, num_examples=args.num_examples,
    ...
    text_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=datasettext.collate_fn)
    ...
    if args.resume and os.path.exists(checkpoint_path):
        ...
        if args.feat_model_path is not None and args.feat_model_path.lower() != 'none':
            ...
            miss, unexpo = model.netG.Content_Encoder.load_state_dict(checkpoint['model'], strict=False)
            assert unexpo == ['fc.weight', 'fc.bias']
            miss, unexpo = model.netG.Content_Encoder.load_state_dict(checkpoint['model'], strict=False)
            assert unexpo == ['fc.weight', 'fc.bias']

            miss, unexpo = model.netTID.Feat_Encoder.load_state_dict(checkpoint['model'], strict=False)
            assert unexpo == ['fc.weight', 'fc.bias']
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.wandb: wandb.log({
            ...
            'loss-loss_TID_fake': losses['TID_fake'],
            'loss-loss_TID_real': losses['TID_real'],
            'epoch': epoch,
        ...
        if epoch % args.save_model == 0 and args.test is False:
            torch.save(checkpoint, os.path.join(MODEL_PATH, 'model.pth'))
        if epoch % args.save_model_history == 0 and args.test is False:
