...
class TextDatasetText():
    ...
    def __init__(self, base_path, collator_resolution, num_examples=15, target_transform=None, min_virtual_size=0):
        ...
        self.TEXT_IMG = pickle.load(file_to_store)
        self.TEXT_IMG = {entry['img_id']: entry for entry in self.TEXT_IMG}
        self.TEXT_IMG = dict(list(self.TEXT_IMG.items()))
        self.img_id = list(self.TEXT_IMG.keys())

        # self.alphabet = ''.join(sorted(set(''.join(d['label'] for d in sum(self.TEXT_IMG.values(), [])))))
        self.transform = get_transform(grayscale=True)
        self.target_transform = target_transform

        self.collate_fn = TextCollatorText(collator_resolution)

    def __len__(self):
        return len(self.img_id)

    def __getitem__(self, index):
        text_idx = np.random.choice(len(self.img_id))
        text_label = self.TEXT_IMG[text_idx]['label'].encode()
        text_img = np.array(self.TEXT_IMG[text_idx]['img'].convert('L'))
...
class TextCollatorText(object):
    def __init__(self, resolution):
        self.resolution = resolution

    def __call__(self, batch):
        item = {}
        if 'timg' in batch[0].keys():
            timgs = [item['timg'] for item in batch]
            timgs = np.array(timgs)
            timgs = torch.Tensor(timgs)
            item['timg'] = timgs
        if 'tlabel' in batch[0].keys():
            tlabels = [item['tlabel'] for item in batch]
            item['tlabel'] = tlabels
        if 'tidx' in batch[0].keys():
            tidxs = [item['tidx'] for item in batch]
            item['tidx'] = tidxs
        return item
...
class CollectionTextDataset(Dataset):
    def __init__(self, datasets, datasets_path, dataset_class, **kwargs):
        # 参数 __init__(self, "IAM/CVL", "files", TextDataset/TextDatasetval, **kwargs):
...
class FidDatasetLong(FidDatasetNum):
    def __init__(self, *args, threshold=1000, **kwargs):
        super().__init__(*args, **kwargs)
        self.chars_freq = {"e": 33548, "t": 23659, "a": 21117, "o": 19330, "n": 18563, "i": 18108, "s": 16496,
                           "r": 16427, "h": 14376, "l": 10598, "d": 10154, "c": 6958, "u": 6831, "m": 6286, "f": 5715,
                           "w": 4860, "y": 4859, "g": 4840, "p": 4815, "b": 3962, ".": 3830, ",": 3098, "v": 2652,
                           "k": 1614, "\"": 980, "-": 961, "'": 953, "M": 925, "T": 921, "I": 813, "A": 784, "S": 660,
                           "B": 562, "P": 556, "H": 549, "W": 513, "C": 498, "N": 493, "G": 443, "R": 435, "x": 409,
                           "L": 390, "E": 375, "D": 335, "F": 325, "0": 316, "1": 281, "j": 276, "O": 222, "q": 216,
                           "U": 132, "K": 125, "(": 114, "3": 113, "?": 113, "9": 112, ")": 111, "z": 109, "2": 107,
                           "J": 105, "V": 105, ":": 104, "Y": 104, ";": 100, "5": 97, "!": 91, "8": 86, "4": 75,
                           "6": 70, "#": 55, " ": 43, "&": 38, "7": 36, "/": 16, "*": 9, "Q": 8, "X": 8, "Z": 5, "+": 2}
        long_tain = set(c for c, freq in self.chars_freq.items() if freq < threshold)
        self.IMG_DATA = {wid: [s for s in samples if len(set(s['label']) & long_tain)] for wid, samples in
                         self.IMG_DATA_OLD.items()}
        self.IMG_DATA = {wid: samples for wid, samples in self.IMG_DATA.items() if len(samples) > 0}
        self.alphabet = ''.join(sorted(set(''.join(d['label'] for d in sum(self.IMG_DATA.values(), [])))))
        self.author_id = sorted(self.IMG_DATA.keys())
        self.dataset_size = sum(len(samples) for samples in self.IMG_DATA.values())
