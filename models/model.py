...
from data.ImageToPickle import ttfToImage
import cv2
...
def printShape(temp, isTamp=False):
    """
    测试输出
    """
    try:
...
def match_dimensions(tensor1, tensor2):
    # 获取两个张量的形状
    shape1 = tensor1.shape
    shape2 = tensor2.shape

    # 找到最大的最后一个维度大小
    max_last_dim = max(shape1[-1], shape2[-1])

    # 计算需要补全的尺寸
    pad_size1 = max_last_dim - shape1[-1]
    pad_size2 = max_last_dim - shape2[-1]

    # 在最后一个维度上对小张量进行补全
    if pad_size1 > 0:
        pad1 = torch.zeros(shape1[:-1] + (pad_size1,), dtype=tensor1.dtype, device=tensor1.device)
        tensor1 = torch.cat([tensor1, pad1], dim=-1)
    if pad_size2 > 0:
        pad2 = torch.zeros(shape2[:-1] + (pad_size2,), dtype=tensor2.dtype, device=tensor2.device)
...
class Generator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.content_encoder = TransformerEncoder(encoder_layer, self.args.tn_enc_layers, encoder_norm)
        ...
        self.Content_Encoder = models.resnet18(weights='ResNet18_Weights.DEFAULT')
        self.Content_Encoder.conv1 = nn.Conv2d(INP_CHANNEL, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.Content_Encoder.fc = nn.Identity()
        self.Content_Encoder.avgpool = nn.Identity()

        self.add_position = PositionalEncoding(dropout=0.1, dim=self.args.tn_hidden_dim)
        self.query_embed = nn.Embedding(self.args.vocab_size, self.args.tn_hidden_dim)
        self.query_embed_MuS = UnifontModule(
            self.args.tn_dim_feedforward,
    ...
    def compute_style(self, ST):
        ...
        Args:
            ST.shape: torch.Size([8, 15, 32, 192])
            FEAT_ST1.shape: torch.Size([120, 3072])
            FEAT_ST2.shape: torch.Size([8, 512, 1, 90])
            FEAT_ST_ENC.shape: torch.Size([90, 8, 512])
            memory.shape: torch.Size([90, 8, 512])
        Returns:torch.Size([90, 8, 512])
        ...
        # memory = self.encoder(FEAT_ST_ENC)
        B, N, R, C = ST.shape
        FEAT_ST1 = self.Feat_Encoder(ST.view(B * N, 1, R, C))
        # printShape(FEAT_ST1)
        FEAT_ST2 = FEAT_ST1.view(B, 512, 1, -1)
        FEAT_ST_ENC = FEAT_ST2.flatten(2).permute(2, 0, 1)
        # FEAT_ST_ENC = self.add_position(FEAT_ST_ENC)
        memory = self.encoder(FEAT_ST_ENC)
        return memory

    def compute_content(self, ST):
        """
        Args:
            ST.shape: torch.Size([8, 1, 32, 192])
            FEAT_ST1.shape: torch.Size([8, 3072])
            FEAT_ST2.shape: torch.Size([8, 512, 1, 6])
            FEAT_ST_ENC.shape: torch.Size([6, 8, 512])

        Returns:torch.Size([6, 8, 512])

        """
        B, N, R, C = ST.shape
        FEAT_ST1 = self.Content_Encoder(ST.view(B * N, 1, R, C))
        # printShape(FEAT_ST1)
        FEAT_ST2 = FEAT_ST1.view(B, 512, 1, -1)
        FEAT_ST_ENC = FEAT_ST2.flatten(2).permute(2, 0, 1)
        # FEAT_ST_ENC = self.add_position(FEAT_ST_ENC)
        memory = self.content_encoder(FEAT_ST_ENC)
        char_emb = torch.mean(memory, 0)  # [N, 512]
        memory = repeat(char_emb, 'n c -> t n c', t=1)

        return memory

    def Eval(self, ST, QRS, QRS_pos, QRSt=None, concat=True):
        """
            Args:
        """
        if self.args.is_seq:
        ...
        for i in range(QRS.shape[1]):
            ...
            if QRSt is None:
                ...
                QRSt_N = QRSt.repeat(N, 1, 1, 1)
                QRt = torch.unsqueeze(QRSt_N[:, i, :], dim=1)
                QRt_EMB = self.compute_content(QRt)
                Tt, Nt, Ct = QRt_EMB.shape
                if concat:
                    if N == Nt:
                        EMB = torch.cat((QRt_EMB, QR_EMB), 0)
                    # else:
                    #     printShape(QR_EMB)
                    #     printShape(QRt_EMB)
                    #     return
                    tgt_mask = generate_square_subsequent_mask(sz=(T + 1)).to(EMB)
                    # tgt = torch.zeros_like(EMB)
                    tgt = self.add_position(EMB)
                    hs = self.decoder(tgt, memory, tgt_mask=tgt_mask)
                else:
                    zero_QR_EMB = torch.zeros_like(QR_EMB)
                    if N == Nt:
                        EMB = torch.cat((QRt_EMB, zero_QR_EMB), 0)
                    # else:
                    #     printShape(QR_EMB)
                    #     printShape(QRSt)
                    #     printShape(QRSt_N)
                    #     printShape(QRt)
                    #     printShape(zero_QR_EMB)
                    #     printShape(QRt_EMB)
                    #     return
                    tgt_mask = generate_square_subsequent_mask(sz=(T + 1)).to(EMB)
                    # tgt = torch.zeros_like(EMB)
                    tgt = self.add_position(EMB)
                    hs = self.decoder(tgt, memory, tgt_mask=tgt_mask)
                    # tgt = torch.zeros_like(QRt_EMB)
                    # hs = self.decoder(tgt, memory, query_pos=QRt_EMB)
    ...
    def forward(self, ST, QR, QRs=None, QR_pos=None, QRs_pos=None, QRt=None, mode='train', concat=True):
        """
        Args:
        """
        # Attention Visualization Init
        ...
        memory = self.compute_style(ST)  # ST: torch.Size([8, 15, 32, 192])
        ...
        # QR_EMB = self.query_embed.weight[QR].permute(1, 0, 2)
        # QR_EMB = self.query_embed(QR).permute(1, 0, 2)  # QR: torch.Size([8, 16])
        # QR_EMB: torch.Size([16, 8, 512])
        # print(self.args.tn_dim_feedforward)
...
class MuS(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.netTID = TextImageDistance(self.args).to(self.args.device)
        self.TID_criterion = TID(self.args)
        ...
        self.optimizer_TID = torch.optim.Adam(self.netTID.parameters(),
                                              lr=self.args.tid_lr, betas=(0.0, 0.999), weight_decay=0, eps=1e-8)
        ...
        self.loss_TID_fake = 0
        self.loss_TID_real = 0
        self.loss_w_fake = 0
        self.loss_w_real = 0
        self.Lcycle = 0
        self.lda1 = 0
        ...
        with open('mytext.txt', 'r', encoding='utf-8') as f:
            ...
            self.text = ''.join(c for c in self.text if c in (
                    self.args.alphabet + self.args.special_alphabet))  # just to avoid problems with the font dataset
            self.text = [word.encode() for word in self.text.split()]  # [:args.num_examples]
        if self.args.textEncoder:
            self.ttfImg = ttfToImage('', words=self.text)
            self.eval_textImg_encode = self.ttfImg.getTextItem()
    ...
    #         self.fakes = self.netG.Eval(ST, self.eval_text_encode, self.eval_encode_pos, concat=self.args.text_concat)
    #         fake_images = torch.cat(self.fakes, 1).detach().cpu().numpy()
    #         real_images = data['img'].detach().cpu().numpy()
    #         writer_ids = data['wcl'].int().tolist()
    #
    #         for i, (fake, real, wid, lb, img_id) in enumerate(
    #                 zip(fake_images, real_images, writer_ids, data['label'], data['idx'])):
    #             print(fake.shape)
    #             return
    #             lb = lb.decode()
    ...
    #         if step % 100 == 0:
    #             print(
    #                 f'[{(step + 1) / len(loader) * 100:.02f}%][{counter:05d}] image saved: {real_img_path.name} ETA {eta}')
    #
    #         with open(path / 'ann.json', 'w') as f:
    #             json.dump(ann, f)
    #     return
    def save_images_for_fid_calculation(self, path, loader, split='train', allImg=True, realImg=True):
        if not isinstance(path, Path): path = Path(path)
        path.mkdir(exist_ok=True, parents=True)
        ...
        ann = {}
        start_time = time.time()
        output_file = self.real_base / 'text.txt'
        ...
        for step, data in enumerate(loader):
            ...
            self.textImg = ttfToImage('', words=texts)
            self.textImgList = self.textImg.getTextItem()
            self.eval_textImg_encodeF = self.textImgList['timg'].to(self.args.device)
            if self.args.textEncoder:
                self.fakes = self.netG.Eval(ST, self.eval_text_encode, self.eval_encode_pos, self.eval_textImg_encodeF,
                                            concat=self.args.text_concat)
            else:
            ...
            for i, (fake, real, wid, lb, img_id) in enumerate(
                ...
                ann[step * self.args.batch_size + i] = lb
                if allImg:
                    img_id = '{:03d}_{:05d}.png'.format(wid, img_id)
    ...
    def _generate_page(self, ST, SLEN, eval_text_encode=None, eval_len_text=None, eval_encode_pos=None,
                       eval_textImg_encode=None, lwidth=260, rwidth=980):
        # ST -> Style?
        ...
        if eval_encode_pos is None:
            eval_encode_pos = self.eval_encode_pos
        if eval_textImg_encode == None and self.args.textEncoder:
            # eval_textImg_encode = self.eval_textImg_encode['timg'].repeat(self.args.batch_size, 1, 1, 1).to(
            #     self.args.device)
            eval_textImg_encode = self.eval_textImg_encode['timg'].to(self.args.device)

        text_encode, text_len, _ = self.netconverter.encode(self.args.special_alphabet)
        symbols = self.netG.query_embed_MuS.symbols[text_encode].reshape(-1, 16, 16).cpu().numpy()
        ...
        if self.args.textEncoder:
            self.fakes = self.netG.Eval(ST, eval_text_encode, eval_encode_pos, eval_textImg_encode,
                                    concat=self.args.text_concat)
        else:
            self.fakes = self.netG.Eval(ST, eval_text_encode, eval_encode_pos)
        page1s = []
        ...
        for batch_idx in range(ST.shape[0]):
            ...
            for idx, fake_ in enumerate(self.fakes):
                ...
                word_t.append(
                    (fake_[batch_idx, 0, :, :eval_len_text[idx] * self.args.resolution].cpu().numpy() + 1) / 2)

                word_t.append(gap)

                if sum(t.shape[-1] for t in word_t) >= rwidth or idx == len(self.fakes) - 1 or (
                        len(self.fakes) - len(self.args.special_alphabet) - 1) == idx:
    ...
    def get_current_losses(self):
        ...
        losses['KLD'] = self.KLD
        losses['TID_fake'] = self.loss_TID_fake
        losses['TID_real'] = self.loss_TID_real
        return losses
    ...
    def _set_input(self, input, input2=None):
        self.input = input
        self.input_t = input2
    ...
    def forward(self):
        ...
        if self.input_t is not None:
            self.tdata = self.input_t['timg'].to(self.args.device)
            self.words = self.input_t['tlabel']
        else:
            self.tdata = None
        ...
        self.ttfImg_label = ttfToImage('', words=self.label)
        self.textImg_item = self.ttfImg_label.getTextItem()
        self.textImg_label = self.textImg_item['timg'].unsqueeze(1).to(self.args.device)

        self.one_hot_real = make_one_hot(self.text_encode, self.len_text, self.args.vocab_size).to(
            self.args.device).detach()  # one_hot_real.shape:torch.Size([8, 8, 80])

        self.text_encode = self.text_encode.to(self.args.device).detach()
        self.len_text = self.len_text.detach()

        # self.words_R = [word.encode('utf-8') for word in np.random.choice(self.lex, self.args.batch_size)]  #随机生成单词label
        # print(self.words_R)
        # print(self.words)
        ...
        for _ in range(self.args.num_words - 1):
            ...
            self.text_encode_fake_j, self.len_text_fake_j, self.encode_pos_fake_j = self.netconverter.encode(
                self.words_j)
        ...
        if self.args.textEncoder:
            self.fake, self.style = self.netG(self.sdata, self.text_encode_fake, self.text_encode_fake_js,
                                              self.encode_pos_fake, self.encode_pos_fake_js, self.tdata,
                                              concat=self.args.text_concat)
        else:
            self.fake, self.style = self.netG(self.sdata, self.text_encode_fake, self.text_encode_fake_js,
                                              self.encode_pos_fake, self.encode_pos_fake_js)
    ...
    def backward_D_OCR(self):
        ...
        if not self.args.no_tid_loss:
            real_img, label_img = match_dimensions(self.real.detach(), self.textImg_label.detach())

            # feat_real, feat_lable = self.netTID(real_img.detach(), label_img.detach())
            # loss_TID_real = self.TID_criterion(feat_real.detach(), feat_lable.detach())
            loss_TID_real = self.netTID(real_img.detach(), label_img.detach())
            # print(loss_TID_real)
            self.loss_TID_real = torch.mean(loss_TID_real[~torch.isnan(loss_TID_real)])
            loss_total += self.loss_TID_real
    ...
    def optimize_D_WL(self):
        ...
        self.set_requires_grad([self.netTID], False)
    ...
    def backward_G_only(self):
        ...
        if not self.args.no_tid_loss:
            fake_img, text_img = match_dimensions(self.fake, self.tdata.detach())
            # feat_fake, feat_text = self.netTID(fake_img, text_img.detach())
            # loss_TID_fake = self.TID_criterion(feat_fake, feat_text.detach())
            loss_TID_fake = self.netTID(fake_img, text_img.detach())

            # print(loss_TID_fake)
            self.loss_TID_fake = torch.mean(loss_TID_fake[~torch.isnan(loss_TID_fake)])
        ...
        if not self.args.no_tid_loss:
            self.loss_T = self.loss_T + self.loss_TID_fake
        ...
        if not self.args.no_tid_loss:
            grad_fake_TID = torch.autograd.grad(self.loss_TID_fake, self.fake, retain_graph=True)[0]
            self.loss_grad_fake_TID = 10 ** 6 * torch.mean(grad_fake_TID ** 2)
        ...
        if not self.args.no_tid_loss:
            grad_fake_TID = torch.autograd.grad(self.loss_TID_fake, self.fake, create_graph=True, retain_graph=True)[0]
            grad_fake_adv = torch.autograd.grad(self.loss_G, self.fake, create_graph=True, retain_graph=True)[0]
            b = self.gb_bate * torch.div(torch.std(grad_fake_adv), self.epsilon + torch.std(grad_fake_TID))
            self.loss_TID_fake = b.detach() * self.loss_TID_fake
            self.loss_T = self.loss_T + self.loss_TID_fake
        else:
            grad_fake_adv = torch.autograd.grad(self.loss_G, self.fake, create_graph=True, retain_graph=True)[0]
            b = 1
        ...
        if b is None:
            print(self.loss_TID_fake, self.loss_G, torch.std(grad_fake_adv))
        if b > 1000 or b < 0.0001:
            print(f'WARNING: beta > 1000 or beta < 0.0001 - beta={b.item()}')
        ...
        if not self.args.no_ocr_loss:
            ...
            self.loss_grad_fake_OCR = 10 ** 6 * torch.mean(grad_fake_OCR ** 2)
        if not self.args.no_tid_loss:
            grad_fake_TID = torch.autograd.grad(self.loss_TID_fake, self.fake, create_graph=False, retain_graph=True)[0]
            self.loss_grad_fake_TID = 10 ** 6 * torch.mean(grad_fake_TID ** 2)
        ...
        if not self.args.no_ocr_loss:
            ...
            if any(torch.isnan(loss_OCR_fake)) or torch.isnan(self.loss_G):
                ...
                sys.exit()
        # print(loss_TID_fake.dim())

        if not self.args.no_tid_loss:
            if any(torch.isnan(loss_TID_fake)) or torch.isnan(self.loss_G):
                print('loss TID fake: ', loss_TID_fake, ' loss_G: ', self.loss_G, ' words: ', self.words)
                sys.exit()
    ...
    def backward_G(self):
        ...
        self.loss_T = self.loss_G_ + self.loss_OCR_fake + self.loss_TID_fake
        ...
        if not False:
            ...
            b = self.opt.gb_bate * torch.div(torch.std(grad_fake_adv), self.epsilon + torch.std(grad_fake_TID))
            ...
            self.loss_T = (
                                  1 - 1 * self.opt.onlyOCR) * self.loss_G_ + self.loss_OCR_fake + self.loss_TID_fake  # + self.loss_w_fake
            self.loss_T.backward(retain_graph=True)
            grad_fake_OCR = torch.autograd.grad(self.loss_OCR_fake, self.fake, create_graph=False, retain_graph=True)[0]
            grad_fake_TID = torch.autograd.grad(self.loss_TID_fake, self.fake, create_graph=False, retain_graph=True)[0]
    ...
    def optimize_D_OCR(self):
        ...
        self.set_requires_grad([self.netTID], True)
        self.optimizer_D.zero_grad()
