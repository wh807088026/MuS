...
class strLabelConverter(object):
    ...
    def encode(self, text):
        ...
        for item in text:
            ...
            result = []
        # print(results)
        return torch.nn.utils.rnn.pad_sequence([torch.LongTensor(text) for text in results],
                                               batch_first=True), torch.IntTensor(length), None
...
class UnifontLabelConverter:
    ...
    def encode(self, text):
        ...
        return torch.nn.utils.rnn.pad_sequence([torch.LongTensor(text) for text in results],
                                               batch_first=True), torch.IntTensor(length), results_centers

    def decode(self, t, length, raw=False):
        raise NotImplementedError


class TextImageDistance(nn.Module):
    def __init__(self, args):
        super(TextImageDistance, self).__init__()
        leakyRelu = False
        self.args = args
        self.TID_criterion = TID(self.args)
        INP_CHANNEL = self.args.num_examples
        if self.args.is_seq: INP_CHANNEL = 1
        # self.TID_criterion = MSELoss()
    ...
    def forward(self, x1, x2):
        ...
        # feat_tid1 = self.cnn(x1)
        # feat_tid2 = self.cnn(x2)
        feat_tid1 = self.Feat_Encoder(x1)
        feat_tid2 = self.Feat_Encoder(x2)

        loss = self.TID_criterion(feat_tid1, feat_tid2)
        # return feat_tid1, feat_tid2
        return loss

class TID(nn.Module):
    def __init__(self, args):
        super(TID, self).__init__()
        self.args = args
        self.TID_loss = MSELoss(reduction='none')

    def forward(self, text, image):
        x1 = text.view(text.size(0), -1)
        x2 = image.view(image.size(0), -1)
        criterion = self.TID_loss(x1, x2)
        loss = criterion.mean(dim=1)
        return loss
