...
if __name__ == '__main__':
    ...
    # parser.add_argument("-t", "--text", default='May the force be', type=str)

    # parser.add_argument("-t", "--text", default='with you.', type=str)
    parser.add_argument("-t", "--text", default='Next year is a new beginning.', type=str)
    parser.add_argument("--text-path", default=None, type=str, help='Path to text file with texts to generate')
    parser.add_argument("-c", "--checkpoint", default='saved_models/IAM-339-15-E3D3-LR5e-05-bs8-debug/5500_model.pth',
                        type=str)
    ...
    if args.text_path is not None:
        with open(args.text_path, 'r') as f:
            args.text = f.read()
    train_wid_ym = ['00', '01', '02', '292', '293', '295']
    out_put_file = "files/output/"
    style_file = "files/style_samples/"
    for iwid in train_wid_ym:


        output_folder = out_put_file + '{}'.format(args.text)
        ...
        writer = MuS_writer(args.checkpoint, fake_args)
        writer.set_style_folder(style_folder)
        fakes = writer.generate(args.text)
        for i, fake in enumerate(fakes):
            output_filename = f"{os.path.basename(style_folder)}_{i:04d}_{args.text.replace(' ', '_')}.png"
            output_path = os.path.join(output_folder, output_filename)
            cv2.imwrite(str(output_path), fake)
        print('Done')
